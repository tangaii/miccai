...
def test_explicit_training_components_cover_public_producers():
    expected = {
        "classification-head", "detection-head", "multilabel-selector-ranker",
...
def test_legacy_task_dispatch_is_still_deterministic():
    ...
    assert _infer_component("detection", {"image_tokens": np.zeros((1, 256, 2560)), "query_states": np.zeros((1, 2560))}) == "detection-head"
